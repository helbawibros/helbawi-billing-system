...
# ج- صفحة الفاتورة
    ...
    qty_str = st.text_input("العدد", value="") # فارغ تماماً
    ...
    if st.session_state.confirmed and st.session_state.temp_items:
        ...
        st.markdown(f"""
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
                <div style="text-align: right;">
                    <p style="margin: 0; font-size:18px;"><b>الزبون:</b> {cust}</p>
                    <p style="margin: 0; font-size:18px;"><b>رقم الزبون:</b> {cust_id}</p>
                    <p style="margin: 0; font-size:18px;"><b>التاريخ:</b> {now_date}</p>
                </div>
                <div style="text-align: left;">
                    <p style="margin: 0; font-size:18px;"><b>المندوب:</b> {st.session_state.user_name}</p>
                </div>
            </div>
        """, unsafe_allow_html=True)

        # حسابات الضرائب والحسم
        ...
        # حساب VAT (11%) بعد الحسم للأصناف بنجمة فقط
        total_vat = 0
